...
class GRUCell(tf.contrib.rnn.BasicRNNCell):# this was changed form tf.nn.rnn_cell.RNNCell because RNNCell was moved after 1.0 update
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable(name = 'W_r', shape = (self.state_size, self.state_size),
                initializer = tf.contrib.layers.xavier_initializer())
            U_r = tf.get_variable(name = 'U_r', shape = (self.input_size, self.state_size),
                initializer = tf.contrib.layers.xavier_initializer())
            ...
            W_o = tf.get_variable(name = 'W_o', shape = (self.state_size, self.state_size),
                initializer = tf.contrib.layers.xavier_initializer())
            U_o = tf.get_variable(name = 'U_o', shape = (self.input_size, self.state_size),
                initializer = tf.contrib.layers.xavier_initializer())
            ...
            z_t = tf.nn.sigmoid(tf.matmul(inputs, U_z) + tf.matmul(state, W_z) + b_z)
            r_t = tf.nn.sigmoid(tf.matmul(inputs, U_r) + tf.matmul(state, W_r) + b_r)
            o_t = tf.nn.tanh(tf.matmul(inputs, U_o) + tf.matmul(tf.multiply(r_t, state), W_o) + b_o)# why do we call this o_t and not h tilde stuff?
